...
var MiniFileMgr$4=(function(){
	...
	['fs',function(){return this.fs=VVMiniAdapter.window.qg;},'wxdown',function(){return this.wxdown=VVMiniAdapter.window.qg.download;}
	]);
	return MiniFileMgr;
})()
...
var VVMiniAdapter=(function(){
	...
	VVMiniAdapter.enable=function(){
		VVMiniAdapter.init(Laya.isWXPosMsg,Laya.isWXOpenDataContext);
	}
	...
	VVMiniAdapter._onMessage=function(data){
		switch (data.type){
			case "changeMatrix":
		...
		if (data['isLoad']=="opendatacontext"){
			if (data.url){
				MiniFileMgr$4.ziyuFileData[data.url]=data.atlasdata;
				MiniFileMgr$4.ziyuFileTextureData[data.imgReadyUrl]=data.imgNativeUrl;
			}
	...
	VVMiniAdapter.postInfoToContext=function(url,atlaspngUrl,atlasJson){
		...
		if (fileNativeUrl){
			laya.vv.mini.VVMiniAdapter.window.qg.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else {
	...
	VVMiniAdapter.sendSinglePicToOpenDataContext=function(url){
		...
		if (fileNativeUrl){
			laya.vv.mini.VVMiniAdapter.window.qg.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else {
			throw "获取图集的磁盘url路径不存在！";
		}
	}
	...
	VVMiniAdapter.sendJsonDataToDataContext=function(url){
		if (!laya.vv.mini.VVMiniAdapter.isZiYu){
			...
			if (atlasJson){
				laya.vv.mini.VVMiniAdapter.window.qg.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else {
				throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
			}
		}
...
var MiniLocalStorage$4=(function(){
	...
	MiniLocalStorage.setItem=function(key,value){
		try{
			VVMiniAdapter.window.qg.setStorageSync({"key":key,"value":value});
		}
		catch(error){
	...
	MiniLocalStorage.removeItem=function(key){
		VVMiniAdapter.window.qg.deleteStorageSync({"key":key});
	}
			...
			if(!VVMiniAdapter.autoCacheFile){
				...
				if (MiniFileMgr$4.isLocalNativeFile(url)){
					...
					if (!MiniFileMgr$4.isLocalNativeFile(url)&& (url.indexOf("http://")==-1 && url.indexOf("https://")==-1)|| (url.indexOf("http://usr/")!=-1)){
						...
						MiniFileMgr$4.downOtherFiles(url,Handler.create(this,this.onDownLoadCallBack,[url]),url);
					}
				}
			}
		}
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)){
			...
			if (!MiniFileMgr$4.getFileInfo(url)){
				...
				if (tempurl.indexOf("http://usr/")==-1&& (tempurl.indexOf("http://")!=-1 || tempurl.indexOf("https://")!=-1)&& !VVMiniAdapter.AutoCacheDownFile){
				...
				var fileObj=MiniFileMgr$4.getFileInfo(url);
				fileObj.encoding=fileObj.encoding==null ? "utf8" :fileObj.encoding;
				MiniFileMgr$4.readFile(url,fileObj.encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url);
			}
		}
	}
	...
	MiniLoader.onReadNativeCallBack=function(encoding,url,type,cache,group,ignoreCache,thisLoader,errorCode,data){
		...
		if (!errorCode){
			...
			if(!VVMiniAdapter.isZiYu &&VVMiniAdapter.isPosMsgYu && type !=/*laya.net.Loader.BUFFER*/"arraybuffer"){
