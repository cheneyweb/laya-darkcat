...
var MiniInput$5=(function(){
	function MiniInput(){}
	__class(MiniInput,'laya.qq.mini.MiniInput',null,'MiniInput$5');
...
var MiniLocalStorage$5=(function(){
	...
	__class(MiniLocalStorage,'laya.qq.mini.MiniLocalStorage',null,'MiniLocalStorage$5');
...
var QQMiniAdapter=(function(){
	...
	QQMiniAdapter.enable=function(){
		QQMiniAdapter.init(Laya.isWXPosMsg,Laya.isWXOpenDataContext);
	}
	...
	QQMiniAdapter.init=function(isPosMsg,isSon){
		...
		if (!QQMiniAdapter.isZiYu){
			...
			MiniFileMgr$5.existDir(MiniFileMgr$5.fileNativeDir,Handler.create(QQMiniAdapter,QQMiniAdapter.onMkdirCallBack));
		}
		QQMiniAdapter.systemInfo=/*__JS__ */qq.getSystemInfoSync();
		...
		Browser["_pixelRatio"]=QQMiniAdapter.pixelRatio();
		...
		QQMiniAdapter.window.qq.onMessage(QQMiniAdapter._onMessage);
		Config.useRetinalCanvas=true;
	}

	QQMiniAdapter._onMessage=function(data){
		switch (data.type){
			case "changeMatrix":
		...
		if (data['isLoad']=="opendatacontext"){
			if (data.url){
				MiniFileMgr$5.ziyuFileData[data.url]=data.atlasdata;
				MiniFileMgr$5.ziyuFileTextureData[data.imgReadyUrl]=data.imgNativeUrl;
			}
	...
	QQMiniAdapter.postInfoToContext=function(url,atlaspngUrl,atlasJson){
		...
		if (fileNativeUrl){
			/*__JS__ */qq.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else {
	...
	QQMiniAdapter.sendSinglePicToOpenDataContext=function(url){
		...
		if (fileNativeUrl){
			/*__JS__ */qq.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else {
			throw "获取图集的磁盘url路径不存在！";
		}
	}
	...
	QQMiniAdapter.sendJsonDataToDataContext=function(url){
		if (!laya.qq.mini.QQMiniAdapter.isZiYu){
			...
			if (atlasJson){
				/*__JS__ */qq.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else {
				throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
			}
		}
...
var MiniVideo$3=(function(){
	...
	__class(MiniVideo,'laya.qq.mini.MiniVideo',null,'MiniVideo$3');
...
var MiniImage$5=(function(){
	function MiniImage(){}
	__class(MiniImage,'laya.qq.mini.MiniImage',null,'MiniImage$5');
		...
		if (!MiniFileMgr$5.getFileInfo(url)){
			if (url.indexOf('http://usr/')==-1&&(url.indexOf("http://")!=-1 || url.indexOf("https://")!=-1)){
				if(QQMiniAdapter.isZiYu){
					...
					MiniFileMgr$5.downOtherFiles(encodeURI(url),new Handler(MiniImage,MiniImage.onDownImgCallBack,[url,thisLoader]),url);
				}
			}
			else
			MiniImage.onCreateImage(url,thisLoader,true);
...
var MiniFileMgr$5=(function(){
	...
	__class(MiniFileMgr,'laya.qq.mini.MiniFileMgr',null,'MiniFileMgr$5');
...
var MiniLocation$5=(function(){
	...
	__class(MiniLocation,'laya.qq.mini.MiniLocation',null,'MiniLocation$5');
...
var MiniSound$5=(function(_super){
	...
	__class(MiniSound,'laya.qq.mini.MiniSound',_super,'MiniSound$5');
	...
	*/
	__proto.load=function(url){
		...
		if(QQMiniAdapter.autoCacheFile&&MiniFileMgr$5.getFileInfo(url)){
			...
			if(!QQMiniAdapter.autoCacheFile){
				...
				if (MiniFileMgr$5.isLocalNativeFile(url)){
					...
					if (!MiniFileMgr$5.isLocalNativeFile(url)&& (url.indexOf("http://")==-1 && url.indexOf("https://")==-1)|| (url.indexOf("http://usr/")!=-1)){
						this.onDownLoadCallBack(url,0);
						}else{
	...
	__proto.onDownLoadCallBack=function(sourceUrl,errorCode,tempFilePath){
		if (!errorCode){
			...
			if(QQMiniAdapter.autoCacheFile){
				if(!tempFilePath){
					if (MiniFileMgr$5.isLocalNativeFile(sourceUrl)){
						var tempStr=URL.rootPath !="" ? URL.rootPath :URL._basePath;
	...
	*/
	__proto.play=function(startTime,loops){
		...
		if(!tSound)
			return null;
		if(QQMiniAdapter.autoCacheFile&&MiniFileMgr$5.getFileInfo(this.url)){
			var fileNativeUrl;
...
var MiniLoader$5=(function(_super){
	...
	__class(MiniLoader,'laya.qq.mini.MiniLoader',_super,'MiniLoader$5');
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)){
			...
			if(QQMiniAdapter.isZiYu && !MiniFileMgr$5.ziyuFileData[url]){
				url=URL.formatURL(url);
			}
			if(QQMiniAdapter.isZiYu && MiniFileMgr$5.ziyuFileData[url]){
			...
			if (!MiniFileMgr$5.getFileInfo(url)){
				...
				if (tempurl.indexOf("http://usr/")==-1&& (tempurl.indexOf("http://")!=-1 || tempurl.indexOf("https://")!=-1)&& !QQMiniAdapter.AutoCacheDownFile){
					...
					if(fileObj){
						...
						if((url.indexOf("http://")==-1 && url.indexOf("https://")==-1)|| MiniFileMgr$5.isLocalNativeFile(url)){
							...
							MiniFileMgr$5.downFiles(url,encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url,cache);
						}
					}
				}
				}else {
	...
	MiniLoader.onReadNativeCallBack=function(encoding,url,type,cache,group,ignoreCache,thisLoader,errorCode,data){
		...
		if (!errorCode){
			...
			if(!QQMiniAdapter.isZiYu &&QQMiniAdapter.isPosMsgYu && type !=/*laya.net.Loader.BUFFER*/"arraybuffer"){
				/*__JS__ */qq.postMessage({url:url,data:tempData,isLoad:"filedata"});
			}
			thisLoader.onLoaded(tempData);
			}else if (errorCode==1){
			QQMiniAdapter.EnvConfig.load.call(thisLoader,url,type,cache,group,ignoreCache);
...
var MiniAccelerator$5=(function(_super){
	...
	__class(MiniAccelerator,'laya.qq.mini.MiniAccelerator',_super,'MiniAccelerator$5');
	...
	*/
	...
	MiniAccelerator.startListen=function(callBack){
		...
		try{
			QQMiniAdapter.window.qq.onAccelerometerChange(laya.qq.mini.MiniAccelerator.onDeviceOrientationChange);
		}catch(e){}
	}
...
var MiniSoundChannel$5=(function(_super){
	...
	__class(MiniSoundChannel,'laya.qq.mini.MiniSoundChannel',_super,'MiniSoundChannel$5');
	var __proto=MiniSoundChannel.prototype;
	/**@private **/
	__proto.__onEnd=function(){
		MiniSound$5._audioCache[this.url]=this._miniSound;
		if (this.loops==1){
